...
import platform
from os import system, path, popen, name

system("")

# subprocess.run('python --version', stdout=subprocess.PIPE, encoding='utf-8').stdout
py_ver = popen('python --version').read()
...
path = path.join(path.dirname(__file__), 'pipignore.txt')
with open(path) as f:
